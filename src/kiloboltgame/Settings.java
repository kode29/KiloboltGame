...
import kiloboltgame.framework.FileIO;
...
public class Settings {
	...
	public static void save(FileIO files) {
		...
		try {
			...
			// Line by line ("\n" creates a new line), write the value of each
	...
	public static void load(FileIO files) {
		...
		try {
			...
			// Loads values from the file and replaces them with default
			soundEnabled = Boolean.parseBoolean(in.readLine());
			currentLevel = Integer.parseInt(in.readLine());

			// Uses for-loop to load 5 numbers as high scores
			for (int i = 0; i < 5; i++) {
